        ...
        <form autocomplete="off">
            <div class="content-main__header">
                ...
                <div class="form-group">
                    ...
                    <input type="text" name="name" id="name" value="{{ $role->name }}" placeholder="Staff Manager" class="form-control input__field">
                </div>
            </div>
                ...
                @foreach($premissionsParent as $premissionsParentItem)
                    <div class="form-group all__options">
                        ...
                        @foreach($premissionsParentItem->condition as $permissionsChildrenItem)
                            <div class="option__choose">
                                <input type="checkbox" class="selectedId selectedItem{{  $premissionsParentItem->id }}" {{ $permissionsChecked->contains('id',$permissionsChildrenItem->id) ? 'checked' : '' }} id="permision{{ $permissionsChildrenItem->id }}" value="{{$permissionsChildrenItem->id}}" name="permission_id">
...
@section('js')
    ...
    <script>
        function update{{ $prefixView }}(id) {
            ...
            callAjax(
                'PUT',
                '{{ route('administrator.roles.update') }}?id='+id,
                {
